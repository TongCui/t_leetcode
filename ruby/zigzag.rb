...
def convert(s, num_rows)
  ...
  div, mod = s.size.divmod(group_count)
  # puts "div mod is #{[div, mod]}"
  num_cols = (num_rows - 1) * (div + (mod == 0 ? 0 : 1)) 
  matric = Array.new(num_rows){ Array.new(num_cols) { "" } }

  # puts "matric size [#{matric.size}][#{matric.first.size}]"
